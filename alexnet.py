...
#############################################################
# RESNET卷积，用于补充深度
W_conv11 = weight_variable([1, 1, 1, 1])    # 定义一个权值变量，卷积核大小为1*1,1个输入通道，4个输出通道（4个卷积核，提取4种特征）
b_conv11 = bias_variable([1])       # 为每个输出配置一个偏置
h_conv11 = leakyrelu(conv2d(x_image, W_conv11) + b_conv11)    # 卷积并加上偏置，leakyrelu作为激活函数，h_conv1记录了第一层卷积的效果

#################################################################################
# 实现卷积层，共3个
W_conv1 = weight_variable([1, 1, 1, 1])
b_conv1 = bias_variable([1])
h_conv1 = leakyrelu(conv2d(x_image, W_conv1) + b_conv1)
h_pool1 = max_pooling_30x30(h_conv1)
h_norm1 = norm(h_pool1, lsize=4)

W_conv2 = weight_variable([3, 3, 1, 1])
b_conv2 = bias_variable([1])
h_conv2 = leakyrelu(conv2d(h_pool1, W_conv2) + b_conv2)
h_pool2 = max_pooling_30x30(h_conv2)
h_norm2 = norm(h_pool2, lsize=4)

W_conv3 = weight_variable([30, 30, 1, 1])
b_conv3 = bias_variable([1])
h_conv3 = leakyrelu(conv2d(h_pool2, W_conv3) + b_conv3)
h_pool3 = max_pooling_30x30(h_conv3)
h_norm3 = norm(h_pool3, lsize=4)

# 分支合并
# output_F1 = tf.concat([h_branch1, h_branch222, h_branch3_4, h_branch4], 3)  # 通道上合并

# 合并
output_F1 = tf.add(h_conv11, h_norm3)    # resnet合并


# 全连接层（两层隐含层分别为600,300神经元）  flatten layer
h_norm3_flat = tf.reshape(output_F1, [-1, 120 * 240 * 1])  # 将卷积输出的4D矩阵转换为1D向量以输入全连接层，-1代表不限制样本数量
...
# 测试集参数评估设置   for testing accuracy calculation
...
batch_size = 50  # 随机取batch_size个训练样本    batch = 50 for every time training
...
for train_index, test_index in KF.split(decs_keys):
    ...
    with tf.Session(config=config) as sess:
        ...
        plt.savefig('./pictures/alexnet-No {}.png'.format(run_num), bbox_inches='tight')
        plt.pause(3)  # 图片显示3秒
        plt.close()
